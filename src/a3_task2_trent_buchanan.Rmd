...
# What about the droid attack on the wookies? Ki-Adi-Mundi's name was lost after the second dash. 
# Also, Plo-Koon's birth year is incorrect.
...
mpg_make_model <- mpg %>% # Start with mpg
  unite(col = make_model, # Create a new column called 'make_model'
        ...
        sep = "what about the droid attack on the wookies" # Puts "HEY!" between the pieces (default is underscore)
        ) 
...
some_dates <- c(09091997, 04021998, 04232001)
...
gasol_hook_months <- gasol_hookshots %>% 
  count(month)
...
storms_danger <- storms %>% 
  mutate(danger = case_when(
    ...
    !status %in% c("hurricane") ~ "moderate"
  ))
```
